...
describe("Category Page Logic", () => {
  ...
  describe("Category Utils", () => {
    ...
    it("should filter posts by category correctly", () => {
      const frameworkPosts = filterPostsByCategory(mockPosts, "Framework", translations);
      const languagePosts = filterPostsByCategory(mockPosts, "Langage", translations);
      const stylingPosts = filterPostsByCategory(mockPosts, "Style", translations);
      const backendPosts = filterPostsByCategory(mockPosts, "Backend", translations);
  ...
  describe("Static Path Generation Logic", () => {
    it("should generate paths for all categories and languages", () => {
      const allPosts = [
        ...
        // Ajouter quelques articles en anglais
        {
          ...mockPosts[0],
          slug: "astro-guide",
          data: { ...mockPosts[0].data, lang: "en" as const, title: "Astro Guide" }
        },
        {
          ...mockPosts[1],
          slug: "typescript-beginners",
          data: { ...mockPosts[1].data, lang: "en" as const, title: "TypeScript for Beginners" }
        }
      ...
      for (const lang of ["en", "fr"]) {
        ...
        for (const category of categories) {
          ...
          const categoryPosts = filterPostsByCategory(postsInLang, category, langTranslations);
          
          paths.push({
            params: { category: urlCategory },
            props: { category, posts: categoryPosts, lang, translations: langTranslations }
          });
        }
      }
    ...
    it("should ensure each path has the correct structure", () => {
      ...
      for (const category of categories) {
        const urlCategory = normalizeCategoryForUrl(category);
        const categoryPosts = filterPostsByCategory(mockPosts, category, translations);

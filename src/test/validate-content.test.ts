...
describe('Content Validation Utils', () => {
  ...
  describe('validateCanonicalSlugs', () => {
      ...
      const mockPosts: BlogPost[] = [
        {
          collection: 'blog',
          id: 'en/first-post.mdx',
          data: { translationId: 'welcome', canonicalSlug: 'welcome-to-our-blog' }
        },
        {
          collection: 'blog',
          id: 'fr/premier-article.mdx',
          data: { translationId: 'welcome', canonicalSlug: 'bienvenue-sur-notre-blog' }
        },
        {
          collection: 'blog',
          id: 'en/simple.mdx',
    ...
    it('should fail when canonical slugs have invalid format', () => {
      const mockPosts: BlogPost[] = [
        {
          ...
          data: { translationId: 'invalid1', canonicalSlug: 'Invalid Post Title' } // espaces et majuscules
        },
        {
          collection: 'blog',
        ...
        },
        {
          ...
          data: { translationId: 'invalid3', canonicalSlug: '-starts-with-hyphen' } // commence par trait d'union
        },
        {
          collection: 'blog',
          id: 'en/ends-invalid.mdx',
          data: { translationId: 'invalid4', canonicalSlug: 'ends-with-hyphen-' } // finit par trait d'union
        }
      ];
      ...
      expect(result.errors.some(error => error.includes('Invalid Post Title'))).toBe(true);
      expect(result.errors.some(error => error.includes('my_post_title'))).toBe(true);
      expect(result.errors.some(error => error.includes('-starts-with-hyphen'))).toBe(true);
      expect(result.errors.some(error => error.includes('ends-with-hyphen-'))).toBe(true);
    });
      ...
      const invalidPosts: BlogPost[] = [
        {
          collection: 'blog',
          id: 'en/post1.mdx',
          data: { translationId: 'post1', canonicalSlug: 'Invalid Slug Format' } // slug invalide
        },
        {
          collection: 'blog',
          id: 'fr/post1.mdx',

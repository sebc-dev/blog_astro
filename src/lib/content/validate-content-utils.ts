 ...
 */
export function validateTranslationIdUniqueness(
  posts: BlogPost[],
): ValidationResult {
  ...
  for (const [translationId, paths] of translationIds.entries()) {
    if (paths.length > MAX_TRANSLATION_COUNT) {
      errors.push(
        `translationId "${translationId}" found in ${paths.length} posts: ${paths.join(", ")}`,
      );
    }
  }
...
export function validateCanonicalSlugs(posts: BlogPost[]): ValidationResult {
  ...
  posts.forEach((post) => {
    ...
    if (!canonicalSlug || canonicalSlug.trim().length === 0) {
      ...
      errors.push(
        `Invalid canonicalSlug "${canonicalSlug}" in ${postPath}: must be lowercase letters, numbers, and hyphens only`,
      );
    }
  });

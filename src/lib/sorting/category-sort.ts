...
/**
 * Sort posts according to the specified sort option
 * @param posts - Array of posts to sort
 * @param sortOption - Sort criteria to apply
...
/**
 * Create DOM element for an article card with XSS protection
 * @param post - Post data to render
 * @param translations - Translation strings for UI elements
 * @returns HTMLElement representing the article card
 */
export function createArticleCardElement(
  post: PostData,
  ...
  article.className =
    "card bg-base-300 border-base-content/10 group h-full overflow-hidden border shadow-lg transition-all duration-300 hover:shadow-xl";
  ...
  title.className =
    "card-title group-hover:text-primary mb-3 line-clamp-2 text-lg leading-tight font-bold transition-colors";
  title.setAttribute("data-cy", "article-title");

  const isfrench = window.location.pathname.includes("/fr/");
  const blogPrefix = isfrench ? "/fr/blog/" : "/blog/";
  const titleLink = document.createElement("a");
  titleLink.href = `${blogPrefix}${post.slug}`;
  ...
  description.className =
    "text-muted-accessible mb-4 line-clamp-2 flex-grow text-sm leading-relaxed";
  ...
  readButton.href = `${blogPrefix}${post.slug}`;
  readButton.className = "btn btn-primary btn-xs";
  readButton.setAttribute(
    "aria-label",
    `${translations.readText}: ${post.data.title}`,
  );
...
/**
 * Render articles grid safely using DocumentFragment for performance
 * @param posts - Array of posts to render
 * @param articlesGrid - DOM element container for the articles grid
 * @param translations - Translation strings for UI elements
 */
export function renderArticlesGrid(
  ...
  posts.forEach((post) => {
    const articleElement = createArticleCardElement(post, translations);
...
export function initializeCategorySort(): void {
  ...
  (() => {
    ...
    const translationsDataElement =
      document.getElementById("translations-data");

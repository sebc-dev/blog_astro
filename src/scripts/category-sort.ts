 ...
 */
export function sortPosts(posts: PostData[], sortOption: SortOption): PostData[] {
 ...
 */
export function createArticleCardElement(post: PostData, translations: Translations): HTMLElement {
  ...
  readButton.setAttribute('aria-label', `${translations.readText}: ${post.data.title}`);
 ...
 * @param translations - Translation strings for UI elements
 */
export function renderArticlesGrid(posts: PostData[], articlesGrid: HTMLElement, translations: Translations): void {
...
export function initializeCategorySort(): void {
  ...
  (() => {
    ...
    function handleSortChange(event: Event): void {
      ...
      renderArticlesGrid(sortedPosts, articlesGrid as HTMLElement, translations);
    }

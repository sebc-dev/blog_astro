...
export function estimateReadingTime(
  post: CollectionEntry<"blog"> | { slug: string; data: { title: string; description: string } },
  lang: string = 'fr'
): number {
...
export function getPostCategory(
  ...
  const t = {
    framework: options?.translations?.framework ?? defaultTranslations.framework,
    language: options?.translations?.language ?? defaultTranslations.language,
    performance: options?.translations?.performance ?? defaultTranslations.performance,
  ...
  if (title.includes("astro") || description.includes("astro")) return t.framework;
  if (title.includes("typescript") || description.includes("typescript")) return t.language;
  if (title.includes("performance") || description.includes("performance")) return t.performance;
...
export function getPostTag(
  ...
  const t = {
    ...
    optimization: options?.translations?.optimization ?? defaultTranslations.optimization,
    bestPractices: options?.translations?.bestPractices ?? defaultTranslations.bestPractices,
    comparison: options?.translations?.comparison ?? defaultTranslations.comparison,
  };
  
  if (title.includes("guide") || title.includes("débuter") || title.includes("beginner")) return t.guide;
  if (title.includes("optimisation") || description.includes("optimiser") || title.includes("optimization")) return t.optimization;
  if (title.includes("bonnes pratiques") || description.includes("pratiques") || title.includes("best practices")) return t.bestPractices;
  if (title.includes("vs") || title.includes("comparaison") || title.includes("comparison")) return t.comparison;
